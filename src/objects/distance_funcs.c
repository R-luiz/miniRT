...
/*   Updated: 2024/03/29 11:35:46 by vmalassi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
float	hit_cylinder_distance(t_object *cylinder, t_lightray ray)
{
	...
	a = vec3_normalize(cy->normal); // Cylinder axis direction

	a_perp = vec3_sub(d, vec3_mul(a, vec3_dot(d, a)));
	b_perp = vec3_sub(co, vec3_mul(a, vec3_dot(co, a)));
	...
	if (discriminant >= 0 && a != 0)
	{
		...
		y0 = vec3_dot(vec3_add(co, vec3_mul(d, t0)), a);
		y1 = vec3_dot(vec3_add(co, vec3_mul(d, t1)), a);
